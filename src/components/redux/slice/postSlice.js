...
export const getPostList = createAsyncThunk("post/getList", async () => {
...
const postSlice = createSlice({
  ...
  reducers: {
    getPostListUser: (state, action) => {
      state.postListUser = state.postList?.post?.filter((element) => {
        return element.user._id === action.payload;
      });
    },
  },
  extraReducers: (builder) => {
    ...
    builder.addCase(postDelete.fulfilled, (state, action) => {
      state.postList.post = state.postList.post.filter((element) => {
        return element.post._id !== action.payload.postId;
      });
    });
    ...
    builder.addCase(getPostList.fulfilled, (state, action) => {
      if (action.payload.success) {
        ...
        state.isError.error = false;
        state.isError.message = null;
        state.postListUser = action.payload.post.filter((element) => {
          return (
            element.user._id === JSON.parse(localStorage.getItem("user")).userId
          );
        });
...
export const { getChildComment, getPostListUser } = postSlice.actions;
